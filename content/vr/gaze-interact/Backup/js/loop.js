...
export function castGaze(eng) {

  let renderer = eng.renderer;
  let tempMatrix = eng.gazeMatrix;
  let raycaster = eng.gazecaster;
  let interactables = eng.interactables;
  let line = eng.gazeLine;
  let box = eng.tempBox;
  let cam= eng.camera;
let distance = 0.5;
      // Get the camera's direction
      const direction = new THREE.Vector3();
      cam.getWorldDirection(direction); // Direction is normalized
  
      // Scale the direction to the desired distance
      direction.multiplyScalar(distance);
  
      // Calculate the new position: camera's position + scaled direction
      const newPosition = new THREE.Vector3();
      cam.getWorldPosition(newPosition); // Camera's position
      newPosition.add(direction);
  
      // Set the object's position
      box.position.copy(newPosition);

  // Function to cast a ray from the XR headset
  ...
  if (intersects.length > 0) {
     ...
     //console.log('Intersected objects:', intersects);
      ...
      // Example: highlight the first intersected object
      intersects[0].object.material.color.set(0xff0000);
  }

    // Update the line's position and orientation to match the ray
    //let lineStart = raycaster.ray.origin.clone();
    //let lineEnd = lineStart.clone().add(raycaster.ray.direction.clone().multiplyScalar(10)); // Extend the ray 10 units forward
    //line.geometry.setFromPoints([lineStart, lineEnd]);  
}
