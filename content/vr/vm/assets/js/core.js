...
export class Engine {
  ...
  // Target FPS and related variables
  #lastFrame = 0;
  #targetFPS = 1;
  ...
  #teleportTarget = {};
  #teleportAt;
  #changingDistricts = false;
  #districtStartedLoading = false;
  #districtLoaded = false;
  #teleportRepositioned = false;
  #teleportBlend = new THREE.Color();
  ...
  setDistrictLoaded(state) { this.#districtLoaded = state; }
  ...
  sessionStarted(session) {
    ...
    this.#teleportFader.userData.isUI = true;
    ...
    this.#world.loadWorldData().then(function() {
      ...
      // Setup and configure the movement manager
      this.#movementManager = new LOOP.MovementManager(
        this,
        this.#controller0,
        this.#controller1,
        this.#scene,
  ...
  prepareTeleport(port) {
    ...
    const district = port.getDistrict();
    this.#changingDistricts = !(district === this.#world.getDistrictId());
    const region = this.#world.getRegionById(port.getRegion());
    this.#teleportTarget = {
      district: district,
      base: (this.#changingDistricts ? port.getTarget() : region.getPosition()),
...
#w;
  #animate() {
    ...
    const dtMs = this.#time - this.#l;
    this.#l = this.#time;

    this.#w = Math.max(this.#w, dtMs);
    if (dtMs > 25) console.log("Spike:", dtMs.toFixed(1), "ms");
    // Throttle the FPS, or at least the running of game logic.
    //const frameDuration = (1000 / this.#targetFPS);
    //if (!this.#lastFrame) {
      //this.#lastFrame = this.#time;
    //}

    //const delta = (this.#time - this.#lastFrame);

    // Determine whether to skip the game logic.
    //let skip = false;
    //if (delta < frameDuration) {
      //skip = true;
    //}

    // Reduce drift by keeping remainder.
    //this.#lastFrame = (this.#time - (delta % frameDuration));
const skip = false;
    if(!skip) {
      ...
      if(this.#canLaunch) {
        ...
        if(this.#isTeleporting) {
          ...
          if(
            !this.#changingDistricts ||
            (this.#changingDistricts && (!this.#districtStartedLoading)) ||
            (this.#changingDistricts && (this.#districtStartedLoading && this.#districtLoaded))
          ) {
            ...
            if(percent >= 0.95 && !this.#teleportRepositioned) {
              ...
              if(this.#changingDistricts) {

                this.#districtStartedLoading = true;
                UTILS.purgeScene(this.#scene);
                ...
                this.#world.loadDistrict(this.#teleportTarget.district);
              }
            }
            ...
            if(percent <= 0.05 && this.#teleportRepositioned) {
              ...
              if(this.#changingDistricts) {
                ...
                this.#changingDistricts = false;
                this.#districtStartedLoading = false;
        ...
        // Check for any movement by the player
        this.#movementManager.checkMovement(this.#delta);

        // Check for any interactions
        this.#interactionManager.checkInteractions(0);
        //this.#interactionManager.checkInteractions(1);
        ...
        if(this.#debugPanel !== null)
          GUI.updateDebugPanel(this.#debugPanel, this.#camera, this.#camPos, this.#camQuat, this.#debugOffset)
