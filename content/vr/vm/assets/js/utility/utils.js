...
const palette = {
  ...
  "black": new THREE.MeshStandardMaterial( { color: 0x000000, roughness: 1, metalness: 0 } ),
...
// Non-destructive removal of a model from a THREE/WebXR scene.
// - Detaches from parent (so it’s truly out of the render lists)
// - Marks it and all descendants invisible & on an unused layer
// - Stops any AnimationMixers found in userData
// - Pauses any VideoTextures found on materials
// - Optionally clears renderer render lists to avoid cached "ghosts"
// Nothing is disposed, so you can re-add the model later.
//
// Usage:
//   removeModelNonDestructive(controllerModel, renderer); // renderer optional
//
// Re-adding later:
//   const park = model.userData.__park || {};
//   (park.parent || scene).add(model);     // choose your parent
//   model.visible = park.visible ?? true;  // restore if you like
//   model.layers.set(park.layer ?? 0);

export function removeModelFromScene(model, renderer) {
  if (!model || typeof model.traverse !== 'function') return;

  // Stash a tiny bit of state for easy restoration later.
  if (!model.userData) model.userData = {};
  if (!model.userData.__park) {
    model.userData.__park = {
      parent: model.parent || null,
      index: model.parent ? model.parent.children.indexOf(model) : -1,
      visible: model.visible,
      layer: model.layers ? model.layers.mask : undefined,
    };
  }

  // Truly detach from the scene graph so it won't be visited during render.
  if (model.parent) {
    model.parent.remove(model);
  }

  // Make it inert: invisible + move to an "unused" layer (31 is a safe bet).
  // (Not strictly necessary after .remove(), but helpful if someone re-attaches
  // it accidentally before you’re ready.)
  const moveToDormantLayer = (obj) => {
    if (obj.layers && typeof obj.layers.set === 'function') obj.layers.set(31);
  };

  // Helper to iterate zero/one/many materials
  const forEachMaterial = (obj, fn) => {
  ...
  // Pause helpers we might encounter
  const pauseVideoTextureIfAny = (mat) => {
    const tryPause = (tex) => {
      if (tex && tex.isVideoTexture && tex.image && typeof tex.image.pause === 'function') {
        // Pause the HTMLVideoElement; do NOT null it.
        try { tex.image.pause(); } catch (_) {}
      }
    };
    tryPause(mat.map);
    tryPause(mat.emissiveMap);
    tryPause(mat.aoMap);
    tryPause(mat.metalnessMap);
    tryPause(mat.roughnessMap);
    tryPause(mat.normalMap);
    tryPause(mat.alphaMap);
    tryPause(mat.displacementMap);
  };

  // Traverse and neutralize everything under the model
  model.traverse((obj) => {
    // Visibility is respected by renderer *and* Raycaster
    obj.visible = false;

    // Keep transforms stable (optional, helps prevent surprising drift if something
    // is still mutating the object elsewhere)
    if ('matrixAutoUpdate' in obj) obj.matrixAutoUpdate = false;

    // Layers: move to a layer most cameras won't render
    moveToDormantLayer(obj);

    // Stop per-object animations if the app stored mixers on userData
    // (common pattern when loading GLBs with animations)
    if (obj.userData && obj.userData.mixer && typeof obj.userData.mixer.stopAllAction === 'function') {
      try { obj.userData.mixer.stopAllAction(); } catch (_) {}
    }

    // Pause any VideoTextures to avoid decoding work and odd “ghost” frames
    forEachMaterial(obj, pauseVideoTextureIfAny);

    // Make sure it won’t cast/receive shadows anywhere it might get reattached
    if ('castShadow' in obj) obj.castShadow = false;
    if ('receiveShadow' in obj) obj.receiveShadow = false;

    // Neutralize any per-frame hooks that might have side effects without disposing them
    // (We’ll keep references so you can restore if you want.)
    if (!obj.userData.__savedHooks) {
      obj.userData.__savedHooks = {
        onBeforeRender: obj.onBeforeRender || null,
        onAfterRender: obj.onAfterRender || null,
      };
    }
  ...
  // Clear renderer’s internal render lists so nothing stale is queued.
  // This does NOT dispose materials/geometries/textures.
...
export function remapColours(model) {
...
export async function loadUnmodifiedGLB(path, renderer) {
...
// Helper: check if an object OR ANY OF ITS PARENTS is flagged as UI
function isUiObject(obj) {
  let cur = obj;
  while (cur) {
    if (cur.userData && cur.userData.isUI) return true;
    cur = cur.parent;
  }
  return false;
}

// Helper: dispose a material and any textures/uniform textures it owns
function disposeMaterial(material) {
  if (!material) return;

  // Dispose any texture-like properties on the material
  ...
  // Dispose textures inside uniforms, if present
...
// Main: purge the scene while skipping UI and preserving InstancedMesh bases
export function purgeScene(scene) {
  const toRemove = [];

  scene.traverse((obj) => {
    // 1) Skip UI objects entirely (and their children)
    //    Anything under a node with userData.isUI = true is left alone.
    if (isUiObject(obj)) return;

    // 2) Special case: InstancedMesh
    if (obj.isInstancedMesh) {
      // We want to KEEP the base InstancedMesh (geometry + material),
      // but remove all instances so it renders nothing.
      if ("count" in obj) {
        // Modern THREE
        obj.count = 0;
      } else if ("maxInstancedCount" in obj) {
        // Older THREE
        obj.maxInstancedCount = 0;
      }

      // Optional: if you really want a "clean slate", you could zero out
      // instanceMatrix/instanceColor here, but it's not required to stop drawing.
      //
      ...
      // Do NOT dispose geometry/material and do NOT remove from scene.
      return;
    }

    // 3) Generic objects: dispose geometry/materials and mark for removal
    ...
    if (obj.material) {
      if (Array.isArray(obj.material)) {
        obj.material.forEach((m) => m && disposeMaterial(m));
      } else {
        disposeMaterial(obj.material);
      }
    }

    // Mark this object to be removed from its parent after traversal
    // (we don't remove during traverse to avoid messing up the iteration)
    if (obj.parent) {
      toRemove.push(obj);
    }
  });

  // 4) Actually remove the non-UI, non-InstancedMesh objects from the scene graph
...
export function disposeObject(obj) {
  // If it’s still in the scene, remove it
  if (obj.parent) {
    obj.parent.remove(obj);
  }

  // InstancedMesh: you might want to keep bases; adjust as needed
  if (obj.isInstancedMesh) {
    // If you want to nuke it completely:
    if (obj.geometry && typeof obj.geometry.dispose === 'function') {
      obj.geometry.dispose();
    }
    if (Array.isArray(obj.material)) {
