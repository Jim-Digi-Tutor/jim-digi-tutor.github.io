...
export function handleControllerInput(gp0, gp1, scene, vec, dolly, player, box, structure, scale) {
  ...
  if(gp0.gamepad !== null && gp1.gamepad !== null) {
    ...
    if(axial) {
      ...
      let newX = (dolly.position.x - (vec.x * (movementSpeed * scale) * val));
      let newZ = (dolly.position.z - (vec.z * (movementSpeed * scale) * val));
