...
export class World {
  ...
  * @param {Number} [detail=2] The level of detail required (0: low, 1: mid, 2: high)
  ...
  getCommonModel(alias, detail = 2) {
    ...
    for(let a = 0; a < this.#commonModels.length; a++) {
      ...
      if(this.#commonModels[a].getAlias() === alias) {
        ...
        if(detail === 0 && this.#commonModels[a].getLowDetail() !== null)
          return this.#commonModels[a].getLowDetail().model;
        else if(detail === 1 && this.#commonModels[a].getMidDetail() !== null)
...
this.getEngine().getScene().add(sun);*/
  ...
  async #loadCommonModels(common, onModelLoaded) {
    ...
    for(let a = 0; a < common.length; a++) {
      ...
      const hModel = await UTILS.loadCommonGlb(file, hMerge, hScale, hRot, hRemap, this.#engine.getRenderer());
      ...
      if(mid !== null) {
        ...
        const mModel = await UTILS.loadCommonGlb(mFile, mMerge, mScale, mRot, mRemap, this.#engine.getRenderer());
      ...
      if(low !== null) {
        ...
        const lModel = await UTILS.loadCommonGlb(lFile, lMerge, lScale, lRot, lRemap, this.#engine.getRenderer());
...
class CommonModel {
  ...
  constructor(alias, highDetail, midDetail, lowDetail) {
