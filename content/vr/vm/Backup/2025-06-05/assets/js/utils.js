...
export function adjustVectorToRegionModel(vec, worldSize, position, baseScale, regionScale) {
  ...
  adj.x = (position.x - (worldSize.x) / 2) + x;
  adj.y = (position.y + y);
  adj.z = (position.z + (-(worldSize.z / 2) + z));
...
export async function loadObj(path, objLoader, mtlLoader, baseScale, adjustPosition, pos, adjustScale, scale, adjustRotation, rot, addTo) {
  ...
  if(adjustScale) {
    object.scale.set(scale.x * baseScale, scale.y * baseScale, scale.z * baseScale);
  } else {
