...
export class InteractionManager {
   ...
   * @param {Number} uniScale The universal scale of the environment.
   * @param {Number} modScale The model scale of the environment.
   ...
   * 
  */
  constructor(engine, controller0, controller1, uniScale, modScale, range, scene, interactables, collidables) {
  ...
  checkInteractions() {
    ...
    if(interactions.length > 0) {
      ...
      if(distance <= this.#range) {
        ...
        let checkStructure1 = this.#ray1.intersectObjects(this.#collidables);
        ...
        if(checkStructure1.length === 0 || checkStructure1[0].distance > distance || checkStructure1[0].object === obj) {
          ...
          this.#nullifyPicked(this.#picked1);
        }
      }
    } else {
  ...
  #nullifyPicked(index) {
    ...
    if(picked.obj !== null) {
      ...
      // However, exclude any screen interface objects from this.
      const data = picked.obj.userData;
      if(!data.hasOwnProperty("screen"))
      ...
      if(data.hasOwnProperty("screen")) {
        ...
        if(action.includes("INFO-BOARD-SCREEN-")) {
          ...
          if(board !== null) {
            ...
            // Prepare the raycaster; it will be needed to identify where on the board the ray was when the button was released
            this.#scene.updateMatrixWorld();
            this.#matrix1.identity().extractRotation(this.#controller1.controller.matrixWorld);
            this.#ray1.ray.origin.setFromMatrixPosition(this.#controller1.controller.matrixWorld);
            this.#ray1.ray.direction.set(0, 0, -1).applyMatrix4(this.#matrix1);
            ...
            // Setup the array to store interactions
            const points = this.#ray1.intersectObject(picked.obj);
            board.onControllerClickDown(picked.obj.worldToLocal(points[0].point.clone()));
          }
        } 
      }      
    }
    ...
    if(data.hasOwnProperty("screen")) {
      ...
      if(action.includes("INFO-BOARD-SCREEN-")) {
        ...
        if(board !== null) {
          ...
          // Setup the array to store interactions
          const points = this.#ray1.intersectObject(obj);
          board.onControllerClickRelease(obj.worldToLocal(points[0].point.clone()));
        }
      } 
    }
  }
