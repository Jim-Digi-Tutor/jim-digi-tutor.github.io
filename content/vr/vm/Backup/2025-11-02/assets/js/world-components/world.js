...
this.getEngine().getScene().add(sun);*/
  ...
  async #loadCommonModels(common, onModelLoaded) {
    ...
    for(let a = 0; a < common.length; a++) {
      ...
      const remap = parseInt(c.querySelector("remap").textContent.trim()) === 0 ? false : true; 
      const rotNode = c.querySelector("rotation");
      const rot = rotNode ? UTILS.getVectorFromXml(rotNode, 0) : null;
      const model = await UTILS.loadCommonGlb(file, merge, scale, rot, remap, this.#engine.getRenderer());
      promises.push({
        name: name,
        file: file,
        merge: merge,
