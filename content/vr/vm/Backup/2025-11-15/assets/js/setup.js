...
new RGBELoader()
  .load('assets/hdr/meadow_2_1k.hdr', (hdrTex) => {
    ...
    //scene.environmentRotation = Math.PI / 2; // 90° rotation
...
renderer.toneMappingExposure = 0.35;
    //scene.environmentRotation = Math.PI / 2; // 90° rotation
        //scene.environment.rotation = THREE.MathUtils.degToRad(180); //Math.PI / 2; // 90° rotation  
        //scene.environmenRotation = THREE.MathUtils.degToRad(180); //Math.PI / 2; // 90° rotation  
...
// Build some simple shapes in controller-local space
const geos = [];
...
tipGeo.translate(0.06, 0, 0); // move to the end of the handle
geos.push(tipGeo);

// Merge into one geometry (single draw call with one material)
const mergedGeo = BGU.mergeGeometries(geos, false);

const controllerMat = new THREE.MeshStandardMaterial({ color: 0x00ffcc });
const controllerProxyMesh = new THREE.Mesh(mergedGeo, controllerMat);
controllerProxyMesh.castShadow = true;
controllerProxyMesh.receiveShadow = true;

// For each controller grip:
//const grip = renderer.xr.getControllerGrip(0);

// Hide or remove the original model if it exists
// grip.remove(originalControllerModel); // or originalControllerModel.visible = false;

// Attach the simple proxy mesh
//grip.add(controllerProxyMesh);

    const group = {};
    group.controller = renderer.xr.getController(index);
    group.grip = renderer.xr.getControllerGrip(index);
    group.gamepad = null;
    group.grip.add(controllerProxyMesh); //factory.createControllerModel(group.grip));
