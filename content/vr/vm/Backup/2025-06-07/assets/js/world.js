...
export class World {
  ...
  async #processRegion(id, data) {
    ...
    if(alias !== "unknown-region") {
      ...
      const size = UTILS.getVectorFromXml(terrain.querySelector("size"), 0);
      const position = UTILS.getVectorFromXml(terrain.querySelector("position"), 0);
      ...
      const files = terrain.getElementsByTagName("terrain-file");
      for(let a = 0; a < files.length; a++) {      

      const load = await UTILS.loadObj(
        ("./assets/models/scene/" + alias + "/terrain/" + files[a].textContent.trim() + ".obj"),
