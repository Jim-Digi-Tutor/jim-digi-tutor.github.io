...
export class World {
  ...
  async #buildWorld(xml) {
    ...
    this.#worldPosition = UTILS.applyVectorToWorld(UTILS.getVectorFromXml(global.querySelector("position"), 0), this.#uniScale);

    // Ascertain the avatar's position and direction
    this.#avatarPosition = UTILS.applyVectorToWorld(UTILS.getVectorFromXml(global.querySelector("avatar-position"), 0), this.#uniScale);
    this.#avatarDirection = UTILS.applyVectorToWorld(UTILS.getVectorFromXml(global.querySelector("avatar-direction"), 0), this.#uniScale);
    ...
    for(let a = 0; a < regions.length; a++) {
      ...
      const region = await this.#processRegion(a, regions[a]);
    }
  }
  ...
  async #processRegion(id, data) {
    ...
    region.setSize(UTILS.getVectorFromXml(data.querySelector("size"), 0));
    ...
    region.setPosition(UTILS.adjustVectorToRegionModel(position, this.#scaledWorld, this.#worldPosition, uniScale, region.getScale()));
    this.#regions.push(region);
    
    if(alias !== "unknown-region") {
      
      /**
       * ADD THE REGION TERRAIN
       */
      const terrain = data.querySelector("terrain");
      const pieces = terrain.getElementsByTagName("terrain-piece");
      for(let a = 0; a < pieces.length; a++) {      
        ...
        const adjPos = UTILS.adjustVectorToRegionModel(position, this.#scaledWorld, this.#worldPosition, uniScale, scale);
        const load = await UTILS.loadObj(
          ("./assets/models/scene/" + alias + "/terrain/" + file + ".obj"),
          new OBJLoader(),
          new MTLLoader(), 
          uniScale,
          false, adjPos,
        ...
        const ter = load.model;
        ter.userData.RegionModelId = ("Region-Terrain-Model-" + id + "-" + a);
        ter.userData.ModelType = "Terrain";
        region.setScaledSize(load.scaled);
        this.#engine.getScene().add(ter);

        // Add the terrain piece to both the terrain and structure arrays
        this.#engine.setTerrain(ter);
        this.#engine.setStructure(ter);
      }
      
      /**
       * ADD THE REGION MODELS
       */
      ...
      for(let a = 0; a < models.length; a++) {
        ...
        //const adjPos = UTILS.adjustVectorToRegionModel(position, this.#scaledWorld, this.#worldPosition, uniScale, scale);
        const adjPos = UTILS.adjustVectorToRegionModel(position, region.getScaledSize(), region.getPosition(), uniScale, scale);

        const load = await UTILS.loadObj(
          ("./assets/models/scene/" + alias + "/models/" + file + "/" + file + ".obj"),
          new OBJLoader(), 
          new MTLLoader(), 
          uniScale,
          false, adjPos,
      ...
      for(let a = 0; a < structures.length; a++) {
        ...
        switch(type) {
          ...
          case "GLB":
            ...
            this.#engine.setStructure(load.model);
